export async function onRequestPost(context) {
  ...
  try {
    ...
    // Use batch() to create table if not exists
    await env.NDB.batch([
      env.NDB.prepare(`
...
// ✅ Handle DELETE request to remove a switch
export async function onRequestDelete(context) {
  ...
  try {
    ...
    // Delete switch
    const res = await env.NDB.prepare(
      "DELETE FROM esp_switches WHERE user_email=? AND label=?"
    ).bind(email, label).run();
